...
Terminal uses real Linux PAM authentication — same as SSH login.
"""
...
try:
    ...
    PAM_AVAILABLE = False
    print("[WARN] python-pam not installed — terminal auth disabled.")

def authenticate(username: str, password: str) -> bool:
    """Authenticate against real Linux users via PAM."""
...
def write_fan_pwm(duty_pct: int):
    ...
    try:
        ...
        print("[ERR] Permission denied writing pwm — add udev rule.")
    except Exception as e:
...
# ── Login route — uses real Linux PAM ─────────────────────────────────────
...
def api_login():
    ...
    if authenticate(username, password):
        print(f"[OK] Terminal login: {username}")
        return jsonify({"ok": True, "username": username})
    else:
        print(f"[WARN] Failed terminal login attempt for: {username}")
        return jsonify({"ok": False, "error": "Invalid username or password"}), 403

# ── Terminal exec — requires valid Linux credentials ───────────────────────
ALLOWED_COMMANDS = {
...
def api_exec():
    ...
    # Re-authenticate on every command for security
    if not authenticate(username, password):
        return jsonify({"error": "Access denied: invalid credentials"}), 403
