...
"""
Plizen v1.3 Backend
- Full PAM authentication with session tokens
- 30 minute inactivity auto-logout
- Full sudo terminal access
- raspi-config style Configure API
...
try:
    ...
    PAM_AVAILABLE = False
    print("[WARN] python-pam not installed — run: pip install python-pam --break-system-packages")
...
# ── Session store ──────────────────────────────────────────────────────────
# token -> {username, password, last_active}
SESSIONS = {}
SESSION_TIMEOUT = 30 * 60  # 30 minutes

def new_session(username: str, password: str) -> str:
...
def require_session():
    """Call at top of every protected route. Returns session or aborts."""
    token = request.headers.get("X-Plizen-Token") or (request.get_json(silent=True) or {}).get("token")
    s = get_session(token)
    if not s:
        return None, jsonify({"error": "Session expired or invalid — please log in again"}), 401
    return s, None, None

def cleanup_sessions():
    while True:
        now = time.time()
        expired = [t for t, s in list(SESSIONS.items()) if now - s["last_active"] > SESSION_TIMEOUT]
        for t in expired:
            del SESSIONS[t]
        time.sleep(60)
...
# ── raspi-config helpers ───────────────────────────────────────────────────
def raspi_get(cmd: list) -> str:
    try: return subprocess.check_output(cmd, text=True, stderr=subprocess.DEVNULL, timeout=5).strip()
    except: return ""

def raspi_set(cmd: list, session) -> dict:
    try:
        env = os.environ.copy()
        result = subprocess.run(
            ["sudo"] + cmd, text=True, timeout=15,
...
def get_configure_data():
    ...
    try:
        cfg = Path("/boot/firmware/config.txt").read_text()
        m = re.search(r"gpu_mem=(\d+)", cfg)
        data["gpu_mem"] = m.group(1) if m else "64"
    except: data["gpu_mem"] = "64"
    return data
...
def api_login():
    ...
    if pam_auth(username, password):
        token = new_session(username, password)
        print(f"[OK] Login: {username}")
        return jsonify({"ok": True, "token": token, "username": username})
    print(f"[WARN] Failed login: {username}")
...
def api_ping():
    ...
    token = data.get("token") or request.headers.get("X-Plizen-Token")
    s = get_session(token)
    return jsonify({"ok": bool(s), "username": s["username"] if s else None})

# Protected stats
@app.route("/api/stats")
def api_stats():
    token = request.headers.get("X-Plizen-Token") or request.args.get("token")
...
def api_disk():
    token = request.headers.get("X-Plizen-Token") or request.args.get("token")
    if not get_session(token): return jsonify({"error": "Unauthorized"}), 401
...
def api_set_fan():
    data = request.get_json()
    if not get_session(data.get("token") or request.headers.get("X-Plizen-Token")):
        return jsonify({"error": "Unauthorized"}), 401
...
def api_fan_curve():
    ...
    if not get_session(data.get("token") or request.headers.get("X-Plizen-Token")):
        return jsonify({"error": "Unauthorized"}), 401
    THERMAL_CURVE = {int(k): int(v) for k, v in data.get("curve",{}).items()}
    return jsonify({"ok": True})
...
def api_exec():
    data = request.get_json()
    session = get_session(data.get("token") or request.headers.get("X-Plizen-Token"))
    if not session:
        return jsonify({"error": "Unauthorized"}), 401
    ...
    try:
        # Run as the authenticated user with sudo available
        result = subprocess.run(
            ["sudo", "-u", session["username"], "-S", "bash", "-c", cmd],
...
def api_set_hostname():
    ...
    hostname = re.sub(r"[^a-zA-Z0-9-]","",data.get("hostname",""))
...
def api_set_interface():
    ...
    try:
        ...
        if remove in cfg:
            cfg = cfg.replace(remove, target)
        elif target not in cfg:
            cfg += f"\n{target}\n"
        subprocess.run(["sudo","tee",cfg_path], input=cfg, text=True, check=True, timeout=10)
        return jsonify({"ok": True, "note": "Reboot required to apply changes"})
    except Exception as e: return jsonify({"error": str(e)}), 500
...
def api_set_gpu_mem():
    ...
    if not get_session(data.get("token") or request.headers.get("X-Plizen-Token")):
        return jsonify({"error": "Unauthorized"}), 401
    mem = int(data.get("gpu_mem", 64))
    if mem not in [16,32,64,128,256]: return jsonify({"error": "Invalid value"}), 400
    try:
        cfg_path = "/boot/firmware/config.txt"
        cfg = Path(cfg_path).read_text()
        if re.search(r"gpu_mem=\d+", cfg):
            cfg = re.sub(r"gpu_mem=\d+", f"gpu_mem={mem}", cfg)
        else:
            cfg += f"\ngpu_mem={mem}\n"
        subprocess.run(["sudo","tee",cfg_path], input=cfg, text=True, check=True, timeout=10)
...
def on_authenticate(data):
    ...
    s = get_session(token)
    if s:
        emit("auth_ok", {"username": s["username"]})
    else:
        emit("auth_fail", {"error": "Session expired"})
...
if __name__ == "__main__":
    ...
    print("[OK] Plizen v1.3 backend → http://0.0.0.0:5000")
