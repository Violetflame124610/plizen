...
# ── Password — written by install script ───────────────────────────────────
...
# ── Sessions — token based, 30 min timeout ─────────────────────────────────
SESSIONS = {}
...
def auth(req=None):
    token = None
    if req:
        token = req.headers.get("X-Plizen-Token")
        if not token:
            try: token = (req.get_json(silent=True) or {}).get("token")
            except: pass
