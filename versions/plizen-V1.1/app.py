...
app = Flask(__name__)
app.config["SECRET_KEY"] = "pi-cockpit-secret"
CORS(app, resources={r"/api/*": {"origins": "*"}})
socketio = SocketIO(app, cors_allowed_origins="*", async_mode="threading")

# ── RPi Active Cooler — hwmon sysfs ───────────────────────────────────────
#
#  The official RPi Active Cooler registers itself under /sys/class/hwmon/hwmonX/
#  Files used:
#    name        → driver name (rpi-pwm-fan, pwmfan, cooling_fan)
#    fan1_input  → live RPM from tachometer  (read-only)
#    pwm1        → PWM duty 0-255            (read/write)
#    pwm1_enable → 0=off 1=manual 2=firmware-auto  (read/write)
#
...
fan_state = {
    ...
    "pct":         None,   # None = firmware is managing
    "auto":        True,   # True = leave to firmware (pwm1_enable=2)
}

# Thermal curve used only when user picks "Curve" mode (not firmware auto)
...
def find_fan_hwmon() -> bool:
    for hwmon in sorted(glob.glob("/sys/class/hwmon/hwmon*")):
        ...
        if name in FAN_HWMON_NAMES:
            ...
            fan_state["name"]        = name
            print(f"[OK] Active cooler found at {hwmon}  (driver: {name})")
            return True
    print("[WARN] Active cooler not found in /sys/class/hwmon — "
          "check connection or dtoverlay.")
    return False
...
def write_fan_pwm(duty_pct: int):
    """Set fan to manual mode and apply duty_pct (0-100)."""
    if not fan_state["pwm_file"]:
        return
    raw = max(0, min(255, int(duty_pct / 100 * 255)))
    try:
        Path(fan_state["enable_file"]).write_text("1")   # manual
        Path(fan_state["pwm_file"]).write_text(str(raw))
        fan_state["pct"]  = duty_pct
        fan_state["auto"] = False
    except PermissionError:
        print("[ERR] Permission denied writing pwm — run as root or add udev rule.")
    except Exception as e:
        print(f"[ERR] write_fan_pwm: {e}")


def set_fan_firmware_auto():
    """Hand control back to firmware (pwm1_enable = 2)."""
    ...
    try:
        ...
        print(f"[ERR] set_fan_firmware_auto: {e}")
...
def calc_net_speed() -> dict:
    ...
    _net_prev = cur; _net_prev_time = now
    return speeds
...
def read_services() -> list:
    ...
    for svc in names:
        try:
            status = subprocess.check_output(["systemctl","is-active",f"{svc}.service"],
                                             text=True,timeout=3).strip()
        except subprocess.CalledProcessError as e:
            status = (e.output or b"").strip() if isinstance(e.output,bytes) else (e.output or "inactive").strip()
        except: status = "unknown"
        try:
            pid = subprocess.check_output(["systemctl","show",f"{svc}.service",
...
def read_journal_logs(n=100) -> list:
    try:
        out = subprocess.check_output(
            ["journalctl","-n",str(n),"--no-pager","-o","json-short","--priority=7"],
            text=True, timeout=5)
...
def read_updates() -> list:
    try:
        out = subprocess.check_output(["apt-get","--simulate","upgrade"],
...
def api_set_fan():
    ...
    pct  = data.get("pct")
    auto = data.get("auto")   # True = firmware auto, False = manual pct
...
ALLOWED_COMMANDS = {
    ...
    "vcgencmd measure_temp","cat /proc/cpuinfo","hostname","ip addr","ps aux","date",
}
...
def api_exec():
    ...
    if cmd not in ALLOWED_COMMANDS:
        return jsonify({"error": f"Not allowed: {cmd}"}), 403
    try:
        out = subprocess.check_output(cmd.split(), text=True, timeout=10,
...
if __name__ == "__main__":
    find_fan_hwmon()
    # Start in firmware-auto mode (safest default)
